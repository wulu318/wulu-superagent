...
test('portal account urls use production base when test mode is disabled', () => {
  ...
  expect(getPortalProfileUrl()).toBe('https://wulu-superagent.com/portal#/profile');
  expect(getPortalRechargeUrl()).toBe('https://wulu-superagent.com/portal#/');
  expect(getPortalInvitationUrl()).toBe('https://wulu-superagent.com/portal#/invitation');
  expect(getPortalCreditsResetActivityUrl()).toBe('https://wulu-superagent.com/portal#/profile?activity=credits_reset');
  expect(getPortalCreditsResetActivityUrl('credits_final_reward_2026_07')).toBe(
    'https://wulu-superagent.com/portal#/profile?activity=credits_reset&campaignCode=credits_final_reward_2026_07',
  );
});
...
test('portal pricing url can include html share keyfrom', () => {
  ...
  expect(getPortalPricingUrl(PortalPricingKeyfrom.HtmlShare)).toBe(
    'https://wulu-superagent.com/portal#/pricing?keyfrom=html_share',
  );
});
