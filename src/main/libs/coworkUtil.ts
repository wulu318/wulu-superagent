...
export function ensureElectronNodeShim(electronPath: string, npmBinDir?: string): string | null {
  try {
    ...
    const nodeShContent = [
      '#!/usr/bin/env bash',
      'if [ -z "${WULU_ELECTRON_PATH:-}" ]; then',
      '  echo "WULU_ELECTRON_PATH is not set" >&2',
      '  exit 127',
      'fi',
      'exec env ELECTRON_RUN_AS_NODE=1 "${WULU_ELECTRON_PATH}" "$@"',
      '',
    ].join('\n');
    ...
    if (process.platform === 'win32') {
      ...
      const nodeCmdContent = [
        '@echo off',
        'if "%WULU_ELECTRON_PATH%"=="" (',
        '  echo WULU_ELECTRON_PATH is not set 1>&2',
        '  exit /b 127',
        ')',
    ...
    if (npmBinDir && existsSync(npmBinDir)) {
      ...
      if (existsSync(npxCliJs)) {
        ...
        // npx.cmd for Windows — uses %WULU_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          const npxCmd = join(shimDir, 'npx.cmd');
          const npxCmdContent = [
            '@echo off',
            '"%~dp0node.cmd" "%WULU_NPM_BIN_DIR%\\npx-cli.js" %*',
            '',
          ].join('\r\n');
          writeFileSync(npxCmd, npxCmdContent, 'utf8');
          coworkLog('INFO', 'resolveNodeShim', `Created npx.cmd shim: ${npxCmd} (using env var WULU_NPM_BIN_DIR)`);
        }
      } else {
      ...
      if (existsSync(npmCliJs)) {
        ...
        // npm.cmd for Windows — uses %WULU_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          ...
          coworkLog('INFO', 'resolveNodeShim', `Created npm.cmd shim: ${npmCmd} (using env var WULU_NPM_BIN_DIR)`);
        }
      } else {
...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in WULU package)
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      '# Auto-generated by WULU – switch Windows console code page to UTF-8',
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  ...
  if (app.isPackaged && !env.WULU_ELECTRON_PATH) {
    env.WULU_ELECTRON_PATH = electronNodeRuntimePath;
  }
  ...
  if (process.platform === 'win32') {
    ...
    if (configuredBashPath && bashPath === configuredBashPath) {
      ...
      if (!configuredHealth.ok) {
        ...
        if (fallbackPath && fallbackPath !== configuredBashPath) {
          ...
          env.WULU_GIT_BASH_RESOLUTION_ERROR = diagnostic;
          coworkLog('WARN', 'resolveGitBash', diagnostic);
          bashPath = null;
        }
      }
    ...
    if (bashPath) {
      ...
      delete env.WULU_GIT_BASH_RESOLUTION_ERROR;
      ...
      env.WULU_GIT_BASH_RESOLUTION_ERROR = truncateDiagnostic(diagnostic);
    }
  ...
  if (shouldInjectShim) {
    ...
    delete env.WULU_NODE_SHIM_ACTIVE;
...
function verifyNodeEnvironment(env: Record<string, string | undefined>): void {
        ...
        if (tool === 'node' && resolvedForExec) {
          try {
            ...
            if (process.platform === 'win32' && /\.cmd$/i.test(resolvedForExec)) {
              execTarget = env.WULU_ELECTRON_PATH || process.execPath;
            }
  ...
  coworkLog('INFO', tag, `WULU_ELECTRON_PATH=${env.WULU_ELECTRON_PATH || '(not set)'}`);
  coworkLog('INFO', tag, `WULU_NPM_BIN_DIR=${env.WULU_NPM_BIN_DIR || '(not set)'}`);
...
export function getSkillsRoot(): string {
  ...
  const envRoots = [process.env.WULU_SKILLS_ROOT, process.env.SKILLS_ROOT]
...
function finalizeNodeToolEnv(env: Record<string, string | undefined>): void {
  if (process.platform === 'win32' || env.WULU_NODE_SHIM_ACTIVE === '1') {
    env.WULU_ELECTRON_PATH = getElectronNodeRuntimePath().replace(/\\/g, '/');
  } else {
    delete env.WULU_ELECTRON_PATH;
  }
}
...
export async function getEnhancedEnv(target: OpenAICompatProxyTarget = 'local'): Promise<Record<string, string | undefined>> {
  ...
  env.WULU_SKILLS_ROOT = skillsRoot; // Alternative name for clarity
  finalizeNodeToolEnv(env);
  await injectSystemProxyForSubprocess(env);
